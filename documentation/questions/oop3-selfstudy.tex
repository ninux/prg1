...
\textit{In the code of TicketMachine, there are two places where you
...
\lstinline{/**}\\
\lstinline{ * Increase score by the given number of points.}\\
\lstinline{ */}\\
\lstinline{public void increase(int points)}\\
\lstinline{\{}\\
\lstinline{    ...}\\
\lstinline{\}}}\\
...
\lstinline{	\}}\\
\lstinline{	// Add the methods here...}\\
\lstinline{\}}\\
}\\

\subsubsection*{Exercise 2.84}
\textit{Add two methods, \lstinline{printAuthor} and \lstinline{printTitle}, 
to the outline \lstinline{Book} class. These should print the 
\lstinline{author} and \lstinline{title} fields, respectively, to the terminal 
window. }\\


\subsubsection*{Exercise 2.85}
\textit{Add a field, \lstinline{pages}, to the \lstinline{Book} class to store 
the number of pages. This should be of type \lstinline{int}, and its initial 
value should be passed to the single constructor, along with the 
\lstinline{author} and \lstinline{title} strings. Include an appropriate 
\lstinline{getPages} accessor method for this field. \\}\\

\subsubsection*{Exercise 2.X}
\textit{Are the \lstinline{Book} objects you have implemented immutable? 
Justify your answer. }\\

\subsubsection*{Exercise 2.86}
\textit{Add a method, \lstinline{printDetails}, to the \lstinline{book} class. 
This should print details of the author, title and pages to the terminal 
window. It is your choice how the details are formatted. For instance, all 
three items could be printed on a single line, or each could be printed on a 
separate line. You might also choose to include some explanatory text to help 
a user work out which is the author and which is the title, for example. \\
\lstinline{Title: Robinson Crusoe, Author: Daniel Defoe, Pages: 232}}\\

\subsubsection*{Exercise 2.87}
