...
\textit{Complete the body of the \verb?setPrice? method so that it assigns the 
value of its parameter to the \verb?price? field. }

\subsubsection*{Exercise 2.33}
\textit{Complete the body of the following method, whose purpose is to add the 
value of its parameter to a field named \verb?score?. \\
\verb?/**
...
of its parameter from a field named \verb?price?. \\
\verb?/**
...
\verb?System.out.println("My cat has green eyes.");?}

\subsubsection*{Exercise 2.37}
\textit{Add a method called \verb?prompt? to the \verb?TicketMachine? class. 
...
\verb?Please insert the correct amount of money. ?}
...
to their \verb?showPrice? methods show the same output, or different? How do 
you explain this effect? }

\subsubsection*{Exercise 2.43}
\textit{Modify the constructor of \verb?TicketMachine? so that it no longer 
...
\textit{Implement a method, \verb?empty?, that simulates the effect of 
removing all money from the machine. This method should have a void return 
type, and its body should simply set the \verb?total? field to zero. Does 
...
then emptying the machine. Is the \verb?empty? method a mutator or an 
accessor? }

\subsubsection*{Exercise 2.46}
\textit{Check that the behavior discussed here is accurate by creating a 
\verb?TicketMachine? instance and calling \verb?insertMoney? with various 
actual parameter values. Check the balance both before and after calling 
\verb?insertMoney?. Does the balance ever change in the cases when an error 
...
\verb?insertMoney? to use the greater-than or equal-to operator: \\
\verb?if(amount >= 0)?\\
...
\verb?boolean? field to control a feature of the circle objects. What was that 
